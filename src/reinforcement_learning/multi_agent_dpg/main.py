...
def obs_list_to_state(observation):
    state = np.array([])
...
def run():
    ...
    memory = MultiAgentReplayBuffer(1_000_000, critic_dims, actor_dims, n_actions, n_agents, 1024)
    ...
    score = evaluate(madpg_agents, parallel_env, episode, total_steps)
    eval_scores.append(score)
    eval_steps.append(total_steps)

    while total_steps < MAX_STEPS:
        obs, _ = parallel_env.reset()
        terminal = [False] * parallel_env.max_num_agents

        while not any(terminal):
            actions = madpg_agents.choose_action(obs)
            ...
            state = obs_list_to_state(list_obs)
            state_ = obs_list_to_state(list_obs_)
...
def evaluate(agents, env, ep, step, n_eval=3):
    ...
    print(f'episode {ep} step {step} score {avg_score}')
    return avg_score
